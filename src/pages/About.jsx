...
import joyLogo from "@/assets/imgs/Joy Snacky logo_border.png";
import img02 from "@/assets/imgs/flyer_1.jpg";
import img03 from "@/assets/imgs/flyer_2.jpg";
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-28 pb-32 text-center">
          <BlurFade delay={0.1}>
            ...
            <p className="max-w-xl mx-auto text-gray-300 font-Montserrat text-lg leading-relaxed">
              From a small campus initiative to a beloved snack destination —
              fueling students, one delicious bite at a time.
            </p>
          </BlurFade>
        </div>
          ...
          <div className="max-w-7xl mx-auto px-4 grid grid-cols-2 md:grid-cols-4 divide-x divide-white/20">
            {STATS.map(({ value, label }) => (
              <div key={label} className="py-8 px-6 text-center">
                <p className="text-3xl md:text-4xl font-bold font-playfair">
                  {value}
                </p>
                <p className="text-sm uppercase tracking-wider mt-1 text-white/80">
                  {label}
                </p>
              </div>
            ))}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
          ...
          <BlurFade delay={0.2}>
            <div className="relative h-[520px]">
              ...
              />
              {/* Logo badge */}
              <div className="absolute top-[38%] left-[45%] -translate-x-1/2 -translate-y-1/2 z-10 w-32 h-32 rounded-full shadow-xl flex items-center justify-center p-2 -2 border-secondary/30">
                <img
                  src={joyLogo}
                  alt="Joy Snacky"
            ...
            <div className="space-y-6 font-Montserrat">
              ...
              <div className="flex items-center gap-4 pt-2">
                ...
                <p className="text-sm text-muted-foreground italic">
                  "Started in a dorm room, now serving the entire campus
                  community."
            ...
            {VALUES.map(({ icon, title, desc, accent }, i) => (
              <BlurFade key={title} delay={0.2 + i * 0.1}>
                <div className="group relative p-8 rounded-2xl border border-white/10 bg-white/5 backdrop-blur-sm hover:bg-white/10 transition-all duration-300 cursor-default overflow-hidden">
                  <div
                    className={`absolute inset-x-0 top-0 h-1 bg-${accent} rounded-t-2xl`}
                  />
                  <div
                    className={`w-14 h-14 rounded-xl bg-${accent}/15 flex items-center justify-center mb-6 text-${accent} group-hover:bg-${accent} group-hover:text-white transition-colors duration-300`}
                  >
                    {icon}
                  </div>
                  <h3 className="text-xl font-bold font-playfair text-white mb-3">
                    {title}
                  </h3>
